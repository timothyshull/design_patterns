...
GasData* GasDataQueue_remove(GasDataQueue* const me)
{
    ...
    if (!isEmpty(me)) {
        ...
        OS_release_semaphore(me->sema);
        return gPtr;
    } else { /* if empty return NULL ptr */ OS_release_semaphore(me->sema);
        return NULL;
    }
}
...
{
    /*
     * this operation computes the next index from the first using modulo
     * arithmetic
     */
