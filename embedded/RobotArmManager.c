...
This function computes a path for the robot arm to follow to position the manipulator at the desired end point. It produces a set of up to 100 actions, each action is a set of
...
/* operation executeStep()
 This operation executes a single step in the chain of actions by executing
 all of the commands within the current action */
...
RobotArmManager* RobotArmManager_Create(void)
{
    ...
    if (me != NULL) { RobotArmManager_Init(me); }
    return me;
}
