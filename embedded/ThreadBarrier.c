...
void ThreadBarrier_synchronize(ThreadBarrier* const me)
{
     ...
     * if so, then release the first blocked thread or the highest priority blocked thread (depending on the OS)
     */
...
ThreadBarrier* ThreadBarrier_Create(void)
{
    ...
    if (me != NULL) { ThreadBarrier_Init(me); }
    return me;
}

void ThreadBarrier_Destroy(ThreadBarrier* const me)
{
    if (me != NULL) { ThreadBarrier_Cleanup(me); }
    free(me);
}