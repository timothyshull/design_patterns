...
void TMDQueue_subscribe(TMDQueue* const me, const UpdateFuncPtr updateFuncAddr)
{
    ...
    pNH = me->itsNotificationHandle;
    if (!pNH) { /* empty list? */
/* create a new Notification Handle, initialize it, and point to it */ printf("-----> Added to a new list\n");
...
/* search list to find end */
        ...
        while (pNH->itsNotificationHandle != NULL) {
            ...
            pNH = pNH->itsNotificationHandle; /* get next element in list */ printf("-----> augmenting ptr\n");
        };
        printf("-----> calling NH_Create\n");
        pNH->itsNotificationHandle = NotificationHandle_Create();
        pNH = pNH->itsNotificationHandle; /* pt to the new instance */ printf("-----> called NH_Create()\n");
    }; /* end if */
/* pNH now points to an constructed Notification Handle */ pNH->updateAddr = updateFuncAddr; /* set callback address */ ++me->nSubscribers;
    printf("-----> wrote updateAddr \n");
    if (pNH->itsNotificationHandle) { printf("xxxxxxx> next Ptr not null!\n\n"); }
    else {
...
int TMDQueue_unsubscribe(TMDQueue* const me, const UpdateFuncPtr updateFuncAddr)
{
    ...
    if (pNH == NULL) { /* empty list? */
        return 0; /* can’t delete it from an empty list */ } else { /* is it the first one? */
        ...
        if (pNH->updateAddr == updateFuncAddr) {
            ...
            while (pNH != NULL) {
                ...
                pNH = pNH->itsNotificationHandle; /* get next element in list */ }; /* end while */
        }; /* end else */
