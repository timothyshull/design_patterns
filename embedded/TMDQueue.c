...
{
/* note that because we never ’remove’ data from this leaky queue, size only increases to the queue size and then stops increasing. Insertion always takes place at the head.
*/
...
{
    TimeMarkedData tmd;
    tmd.timeInterval = -1; /* sentinel values */ tmd.dataValue = -9999;
