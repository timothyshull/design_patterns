...
void CRRobotArmManager_motorZero(CRRobotArmManager* const me)
{
    ...
    if (!success) { CRDisplay_printMsg(me->itsCRDisplay, "Cannot zero!"); }
}
...
void CRRobotArmManager_moveRobotArm(CRRobotArmManager* const me)
{
    ...
    /* note that the function below has its own critical region and so
     * cannot be called inside of the critical region of this function
     */
