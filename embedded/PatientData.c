...
{
    if (PatientData_checkData(me)) { return me->pData.bloodO2Conc; }
    else {
        PatientData_errorHandler(me, CORRUPT_DATA);
        return 0;
    };
...
{
    if (PatientData_checkData(me)) { return me->pData.diastolicBP; }
    else {
        PatientData_errorHandler(me, CORRUPT_DATA);
        return 0;
    };
...
{
    if (PatientData_checkData(me)) { return me->pData.temperature; }
    else {
        PatientData_errorHandler(me, CORRUPT_DATA);
        return 0;
    };
...
static void cleanUpRelations(PatientData* const me)
{
    if (me->itsAlarmManager != NULL) { me->itsAlarmManager = NULL; }
}
