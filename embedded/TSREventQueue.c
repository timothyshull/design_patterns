...
_Bool TSREventQueue_isEmpty(TSREventQueue* const me)
{
    return me->size <= 0;
}
...
/* pull should only be called when there is an event waiting */ Event TSREventQueue_pull(TSREventQueue* const me)
{
    Event e;
    Mutex_lock(me->itsMutex);
