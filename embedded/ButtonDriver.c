...
{
    if (p_Button != NULL) { Button__setItsButtonDriver(p_Button, me); }
...
itsMicrowaveEmitter = p_MicrowaveEmitter;
...
ButtonDriver* ButtonDriver_Create(void)
{
    ...
    if (me != NULL) { ButtonDriver_Init(me); }
    return me;
}

void ButtonDriver_Destroy(ButtonDriver* const me)
{
    if (me != NULL) { ButtonDriver_Cleanup(me); }
    free(me);
}

static void cleanUpRelations(ButtonDriver* const me)
{
    if (me->itsButton != NULL)>itsButton);
    {
    ...
    if (me->itsMicrowaveEmitter != NULL) { me->itsMicrowaveEmitter = NULL; }
    if (me->itsTimer != NULL) {
        me->itsTimer = NULL;
    }
}
