...
/* create the OS event queue and the task */ OSQCreate(&mc_eventQueue[0], 10);
...
panel to set the motor movement settings to set the position of the motors.
Note that if you want to set only one
of the motors, then set a negative value for the commanded position of the other
*/
    m1Pos = rand();
    m2Pos = rand();
    if (m1Pos >= 0) { motor1Pos = m1Pos; }
    if (m2Pos >= 0) { motor2Pos = m2Pos; }
