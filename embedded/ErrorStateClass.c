...
void ErrorStateClass_Init(ErrorStateClass* const me)
{
     ...
     * remember that the initializer for the StateActionSet sets all pointers to NULL as the default
     * Note: In this implementation, pointers are only set to non-NULL for actual handlers (except for entry() and exit()). The caller can tell if the event is handled by checking if the pointer is NULL.
...
int ErrorStateClass_is_in(ErrorStateClass* const me, ErrorStatesType s) { return s == me->currentState; }

struct LightController* ErrorStateClass_getItsLightController(const ErrorStateClass* const me) { return (struct LightController*) me->itsLightController; }

void ErrorStateClass_setItsLightController(ErrorStateClass* const me, struct LightController* p_LightController) { me->itsLightController = p_LightController; }

ErrorStateClass* ErrorStateClass_Create(void)
{
    ErrorStateClass* me = (ErrorStateClass*) malloc(sizeof(ErrorStateClass));
    if (me != NULL) { ErrorStateClass_Init(me); }
    return me;
}

void ErrorStateClass_Destroy(ErrorStateClass* const me)
{
    if (me != NULL) { ErrorStateClass_Cleanup(me); }
    free(me);
}

static void cleanUpRelations(ErrorStateClass* const me) { if (me->itsLightController != NULL) { me->itsLightController = NULL; }}